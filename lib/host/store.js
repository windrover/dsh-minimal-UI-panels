...
export function normalizeRecord(raw, scope) {
  ...
  // Local vector (Stage D): persisted hash-embedding. Kept only when it is a
  // finite-number array so a malformed field can never poison a later search.
  if (Array.isArray(raw.vec) && raw.vec.length > 0 && raw.vec.every((n) => typeof n === 'number' && Number.isFinite(n))) {
    record.vec = raw.vec
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Local hashing-vector embedding (Stage D)
//
// A dependency-free, fully-offline "semantic" vector. Each token is hashed into
// a fixed-width bucket vector (the hashing trick); the text vector is the sum
// of its token buckets, L2-normalized. Two texts that share tokens (including
// CJK bigrams from `tokenize`) land close in cosine space, so synonym / paraphrase
// / reordering matches that lexical BM25 misses become retrievable — with no
// model call and no external service.
//
// This is deliberately a *local* signal, not a true semantic embedding: it ranks
// by lexical-overlap-in-vector-space rather than meaning. But it is zero-cost,
// deterministic, and upgradeable: `embed(text)` is the single seam to swap for a
// real local model (transformers.js / ONNX) later without touching the rest.
// ─────────────────────────────────────────────────────────────────────────────

/** Default vector dimensionality. Powers of two keep bucket spread even. */
export const VECTOR_DIM = 256

/**
 * Stable 32-bit string hash (FNV-1a variant) for bucket assignment.
 */
function hashToken(token) {
  ...
  // mix to unsigned 32-bit
  return h >>> 0
}

/**
 * Embed text into a unit L2-normalized vector of dimension `dim`.
 * @returns a `Float32Array`-like array of length `dim` (all zero for empty text).
 */
export function embedText(text, dim = VECTOR_DIM) {
  const vec = new Array(dim).fill(0)
  const tokens = tokenize(String(text ?? ''))
  if (tokens.length === 0) return vec
  for (const t of tokens) {
    const bucket = hashToken(t) % dim
    // Sign by a second hash so antonym-ish co-occurrence doesn't only add.
 ...
 * if not, the formula still gives a valid unnormalized dot product).
 * @returns a number in [-1, 1] (or 0 for a zero vector).
 */
...
/** Whether a record carries a usable vector. */
export function hasVec(record) {
  return Array.isArray(record.vec) && record.vec.length > 0
}

/**
 * Ensure a record has a `vec` (lazy-embed on read, for records written before
 * Stage D existed). Returns the (possibly newly embedded) vector.
 */
export function ensureVec(record, dim = VECTOR_DIM) {
  if (hasVec(record)) return record.vec
  const v = embedText([record.content, ...(record.tags ?? [])].filter(Boolean).join(' '), dim)
  record.vec = v
  return v
}
...
export class MemoryStore {
  ...
  async put(record) {
    return this.#mutate(() => {
      ...
      // Lazy hash-embed on write (Stage D): keep the record's existing vec if
      // the source already supplied one, otherwise compute from content+tags.
      if (!hasVec(stored)) stored.vec = embedText([stored.content, ...(stored.tags ?? [])].filter(Boolean).join(' '))
  ...
  /**
   * Local hash-vector recall (Stage D): embed the query, then cosine-rank every
   * non-superseded record. Records written before Stage D exist have no `vec`;
   * they are lazily embedded here (and the in-memory copy is upgraded so the
   * next call — or the next persist — keeps it). Pure CPU, no model call.
   * @returns ranked `{ record, score }` (score = cosine ∈ [-1,1]) sliced to `limit`.
   */
  async vectorSearch(query, { limit = 5 } = {}) {
    await this.#ensureLoaded()
    const records = [...this.records.values()].filter((r) => !r.superseded)
    const q = ensureVec({ content: query, tags: [] })
    const scored = []
    for (const record of records) {
      const v = ensureVec(record)
      const s = cosine(q, v)
      if (s > 0) scored.push({ record, score: s })
    }
    scored.sort((a, b) => b.score - a.score || (b.record.hits ?? 0) - (a.record.hits ?? 0))
    ...
    return this.#mutate(() => {
      ...
      for (const op of ops ?? []) {
        if (op.action === 'add') {
          ...
          else if (!hasVec(match.record)) match.record.vec = embedText([match.record.content, ...(match.record.tags ?? [])].filter(Boolean).join(' '))
          tally.replaced += 1
          changed = true
