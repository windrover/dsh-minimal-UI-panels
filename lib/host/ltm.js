    ...
    const routes = {
      ...
      '/api/memory/settings': async (req, res) => {
        if (req.method === 'POST') {
          ...
          const patch = {}
          for (const key of ['autoSummarize', 'compressWithLLM', 'injectContext', 'injectTags', 'requireApprovalForWrite', 'charLimit', 'semanticRecall', 'semanticRerank', 'semanticDedupThreshold', 'autoConsolidate', 'consolidateIntervalMs', 'consolidateMaxEntries']) {
