...
export function apply(ctx, config = {}) {
  ...
  async function semanticRecall({ session, agent, scopes, query, limit, maxCandidates = 12 }) {
    ...
    // Flatten, attaching scope, and cap per-scope candidate pool.
    let candidates = []
    for (const [scope, pool] of byScope) {
      for (const { record, score } of pool.values()) {
        candidates.push({ id: record.id, scope, content: record.content, tags: record.tags ?? [], score })
      }
    }
    ...
    const routes = {
      ...
      '/api/memory/settings': async (req, res) => {
        if (req.method === 'POST') {
          ...
          const patch = {}
          for (const key of ['autoSummarize', 'compressWithLLM', 'injectContext', 'injectTags', 'requireApprovalForWrite', 'autoApproveAfterMs', 'charLimit', 'semanticRecall', 'semanticRerank', 'semanticDedupThreshold', 'autoConsolidate', 'consolidateIntervalMs', 'consolidateMaxEntries']) {
