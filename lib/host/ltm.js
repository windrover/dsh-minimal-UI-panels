    ...
    const routes = {
      ...
      '/api/memory/settings': async (req, res) => {
        if (req.method === 'POST') {
          ...
          const patch = {}
          for (const key of ['autoSummarize', 'compressWithLLM', 'injectContext', 'injectTags', 'requireApprovalForWrite', 'autoApproveAfterMs', 'charLimit', 'semanticRecall', 'semanticRerank', 'semanticVectorRecall', 'semanticDedupThreshold', 'autoConsolidate', 'consolidateIntervalMs', 'consolidateMaxEntries']) {
