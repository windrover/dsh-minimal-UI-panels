...
function apply(ctx, config) {
	...
	// all sessions. Created on `session/created`; the path is also injected
	// into the system prompt so the agent writes into it (see below).
	async function sessionDirFor(session) {
		...
		return join(root, "sessions", `${day}-${id}`);
	}
	ctx.on("session/created", async (session) => {
		try {
			const dir = await sessionDirFor(session);
			if (dir !== null) await fs.mkdir(dir, { recursive: true });
		} catch {
			/* a missing/unwritable root must not break session creation */
		}
	});
