...
async function scanFile(filePath, cfg) {
	...
	return {
		...
		size: stat.size,
		// Rounded on purpose: Node reports mtimeMs as a float (e.g.
		// 1789091760946.6096), but the artifacts_list output schema declares an
		// integer, so an unrounded value fails harness validation and kills the
		// whole tool call. Clients sort and format this as integer ms too.
...
async function readArtifactFile(filePath, cfg, roots) {
	...
	try {
		...
		// Rounded for the same reason as scanFile: mtimeMs must be an integer ms
		// to satisfy the tool's output schema and the client's sort/format code.
		const base = { path: canonical, name: basename(canonical), size: stat.size, mtimeMs: Math.round(stat.mtimeMs) };
...
function apply(ctx, config) {
	...
	// Every session gets its own artifact folder under the workspace root:
	//   <workspace>/sessions/<YYYY-MM-DD>-<sessionId>/
	// so artifacts never pile up in the workspace root and the panel can list
	// all sessions.
	//
	// The folder is NOT pre-created. It used to be, from a `session/created`
	// hook, which looked harmless and was not: `session/created` fires on every
	// resume, not just on first use, so a session that only ever asked questions
	// still left a folder behind each day it was reopened. The workspace filled
	// up with empty directories — 47 of 58 at the time of the fix — one per
	// resume rather than one per session that actually produced something.
	//
	// Nothing needs the folder to exist ahead of time. DSH's own write path
	// creates parent directories (`writeFileAtomic` does
	// `mkdir(dirname, { recursive: true })` in @deepseek-ai/dsh-fs-local), the
	// system-prompt section below tells the agent to `mkdir -p` when it writes
	// with a shell, and `scanWorkspace` already tolerates a missing directory by
	// skipping it. So the directory appears exactly when there is something to
	// put in it.
	//
	// Tell the agent (and the panel) where this session's artifacts belong. The
	// injected path is what the agent should use when writing files, so products
	// from different sessions stay separated and show up per-session.
	ctx.systemPrompt.section({
		name: "tool:artifacts-session-dir",
		order: 117,
		text: (context) => {
			...
			return (
				"Session artifact directory: write this session's output files (scripts, images, " +
				"documents, exports) into the folder `sessions/" + day + "-" + id + "` under the " +
				"workspace root, NOT into the workspace root itself. Create it first if it does not " +
				"exist (mkdir -p). The artifacts panel lists every session's folder, so keeping " +
				"output here keeps it discoverable and separated from other sessions."
			);
		},
	});
