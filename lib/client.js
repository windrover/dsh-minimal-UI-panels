...
const NS = "terminal-notes";
		...
		function TerminalPanel() {
			...
			const termOutput = {
				margin: 0, flex: "2 1 0%", overflow: "auto", whiteSpace: "pre-wrap",
				wordBreak: "break-word", lineHeight: 1.5, minHeight: 0,
			};
			const hintStyle = {
				flex: "1 1 0%", overflow: "auto", fontSize: 11, lineHeight: 1.7,
				color: "#8a94a3", borderTop: "1px solid #2a313a",
				paddingTop: 8, minHeight: 0,
			};
			const hints = [
				"ls -la                   列出文件",
				"pwd                      当前目录",
			...
			return react.createElement("div", { style: termWrap },
				...
				// 常用命令提示常驻：始终渲染在输出区下方（有输出时也在），
				// 可随时参考，不再因执行过命令而消失。
				!error && react.createElement("div", { style: hintStyle },
		...
		function NotesPanel() {
			...
			const splitterStyle = {
				width: 2, flex: "0 0 auto", cursor: "col-resize",
			...
			const rowStyle = {
				display: "flex", gap: 0, height: "100%", minHeight: 0,
			};
		...
		function apply(ctx) {
			...
			// Locale probe: the container passes no locale prop to panels; bind
			// once and reuse the plain t() in components.
			try {
