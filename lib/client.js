...
//#region lib/types/client/locales.js
		...
		const zh = {
			...
			"settings.semanticVectorRecallDesc": "离线哈希向量（余弦）检索与 BM25 用 RRF 融合，抓回同义/改写匹配；零模型调用、零 API、不改 BM25 基线",
		...
		const en = {
			...
			"settings.semanticVectorRecallDesc": "Offline hash-vector (cosine) search, RRF-fused with BM25 — catches synonym/paraphrase matches with no model call and no API (adds one local pass per recall)",
