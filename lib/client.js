...
//#region lib/types/client/locales.js
		...
		const zh = {
			...
			"settings.autoSummarizeDesc": "每轮对话结束后用 LLM 蒸馏值得长期记住的事实（额外模型调用）。｜ 大白话：让 AI 自己把聊天里的重要内容自动存进记忆，不用你手动记。",
			"settings.compressWithLLM": "LLM 压缩",
			"settings.compressWithLLMDesc": "超限时用 LLM 精炼压缩（关闭则用纯规则压缩）。｜ 大白话：记忆快装满时，让 AI 把旧内容压得更短，留地方存新的。",
			"settings.injectContext": "上下文注入",
			"settings.injectContextDesc": "决定每次对话时把多少记忆塞进上下文。recent=只放最新的，full=全放，off=完全不自动注入。｜ 大白话：控制 AI 每次开口前，能想起你多少往事。",
			"settings.requireApproval": "写入审批",
			"settings.requireApprovalDesc": "开启后，AI 想新存记忆时会先放进待确认队列，等你批准才真正写入。｜ 大白话：给记忆加一道把关，AI 不能偷偷往里乱塞东西。",
			"settings.charLimit": "字符预算",
			"settings.charLimitDesc": "记忆库总容量上限（字符数），到顶后会按规则压缩。｜ 大白话：给记忆本子定个最大页数，写满了就整理旧内容。",
			"settings.saved": "已保存",
			"settings.semanticRecall": "语义召回（查询扩展）",
			"settings.semanticRecallDesc": "召回前让 LLM 把查询扩展成同义/改写变体再 BM25 合并，能抓回改写过的记忆（额外模型调用）。｜ 大白话：你换个说法问，AI 也能想起同一件事。",
			"settings.semanticRerank": "语义重排",
			"settings.semanticRerankDesc": "对 BM25 候选让 LLM 打 0–3 相关性分再排序，提升噪声库的精度（额外模型调用）。｜ 大白话：AI 把翻出来的旧记忆按相关程度重新排个序，最贴题的放最前。",
			"settings.semanticVectorRecall": "本地向量召回",
			"settings.semanticVectorRecallDesc": "离线哈希向量（余弦）检索与 BM25 用 RRF 融合，抓回同义/改写匹配；零模型调用、零 API、不改 BM25 基线。｜ 大白话：在电脑本地再算一遍相似度，把意思相近但用词不同的记忆也捞出来，不联网、不花钱。",
			"settings.semanticDedupThreshold": "写入语义去重阈值",
			"settings.semanticDedupThresholdDesc": "autoSummarize 落库前，与既有记忆相似度≥此值则跳过，避免重复堆积（0=关闭，仅精确去重）。｜ 大白话：超过这个相似度就当成「已经记过了」直接跳过，免得同一件事记好多遍。",
			"settings.autoConsolidate": "主动整合",
			"settings.autoConsolidateDesc": "空闲期把近似重复/重叠记忆合并成更少更凝练的条目，避免库只增不减（需开启 LLM 压缩）。｜ 大白话：AI 闲下来时，把零碎的同类记忆并成一条清爽的，笔记不越堆越乱。",
			"settings.consolidateIntervalMs": "整合间隔（毫秒）",
			"settings.consolidateIntervalMsDesc": "距离上次整合超过这么久，就触发一次整理（0=不按时间触发）。｜ 大白话：设定「多久大扫除一次记忆」的间隔。",
			"settings.consolidateMaxEntries": "整合触发条数",
			"settings.consolidateMaxEntriesDesc": "记忆条数超过这个数就触发一次整合。｜ 大白话：记忆攒到多少条，就自动归拢合并一次。",
			"settings.autoApproveAfterMs": "待确认自动放行（毫秒）",
			"settings.autoApproveAfterMsDesc": "开启写入审批后，autoSummarize 蒸馏的事实先进待确认队列；超过此时长未处理则自动落库（0=永不自动）。｜ 大白话：你一直没空看，超过这个时间就自动放行，不用手动批。",
			"panel.pending": "待确认",
		...
		const en = {
			...
			"settings.autoSummarizeDesc": "Distill durable facts with the LLM after each finished turn (extra model call). | Plainly: the AI jots down the important bits from your chat into memory on its own — no manual note-taking.",
			"settings.compressWithLLM": "LLM compression",
			"settings.compressWithLLMDesc": "Refine with the LLM when over budget (off = rule-based compression). | Plainly: when memory gets full, the AI shrinks old entries to make room for new ones.",
			"settings.injectContext": "Context injection",
			"settings.injectContextDesc": "How much memory to feed in before each reply. recent = latest only, full = everything, off = never auto-inject. | Plainly: how much of your past the AI can recall each time it answers.",
			"settings.requireApproval": "Write approval",
			"settings.requireApprovalDesc": "When on, new memories go to a pending queue and only commit after you approve. | Plainly: a gate so the AI can't quietly stuff junk into memory without your OK.",
			"settings.charLimit": "Char budget",
			"settings.charLimitDesc": "Total memory capacity in characters; over the cap it compresses by rule. | Plainly: the max size of the memory notebook — old stuff gets tidied when it fills up.",
			"settings.saved": "Saved",
			"settings.semanticRecall": "Semantic recall (query expansion)",
			"settings.semanticRecallDesc": "Before recall, ask the LLM to expand the query into paraphrase/synonym variants and BM25-merge them — catches rewritten memories (extra model call). | Plainly: word your question differently and the AI still remembers the same thing.",
			"settings.semanticRerank": "Semantic rerank",
			"settings.semanticRerankDesc": "Have the LLM score BM25 candidates 0–3 and re-rank — improves precision on noisy stores (extra model call). | Plainly: the AI re-sorts the recalled memories by relevance, best match first.",
			"settings.semanticVectorRecall": "Local vector recall",
			"settings.semanticVectorRecallDesc": "Offline hash-vector (cosine) search, RRF-fused with BM25 — catches synonym/paraphrase matches with no model call and no API (adds one local pass per recall). | Plainly: it also scores similarity locally, pulling out memories that mean the same even if the words differ — no internet, no cost.",
			"settings.semanticDedupThreshold": "Write dedup threshold",
			"settings.semanticDedupThresholdDesc": "On autoSummarize write, skip a fact whose Jaccard similarity to an existing memory ≥ this — avoids duplicate buildup (0 = off, exact dedup only). | Plainly: above this similarity it treats it as 'already noted' and skips it, so the same thing isn't saved many times.",
			"settings.autoConsolidate": "Auto-consolidation",
			"settings.autoConsolidateDesc": "In idle, merge near-duplicate/overlapping memories into fewer, concise entries so the store stops growing dirty (needs LLM compression on). | Plainly: when idle, the AI merges scattered similar notes into one tidy entry so things don't pile up.",
			"settings.consolidateIntervalMs": "Consolidation interval (ms)",
			"settings.consolidateIntervalMsDesc": "Trigger a cleanup once this long has passed since the last one (0 = don't trigger by time). | Plainly: set how often memory gets a 'spring clean'.",
			"settings.consolidateMaxEntries": "Consolidation trigger count",
			"settings.consolidateMaxEntriesDesc": "Trigger consolidation once the memory count exceeds this. | Plainly: when memory reaches this many entries, merge them down automatically.",
			"settings.autoApproveAfterMs": "Pending auto-approve (ms)",
			"settings.autoApproveAfterMsDesc": "With write approval on, autoSummarize facts wait in a pending queue; older than this are auto-committed (0 = never). | Plainly: if you never get to it, after this long it auto-approves without a manual click.",
...
const NS = "terminal-notes";
		...
		const en = {
			...
			"terminal.cancel": "Stop",
			"terminal.cancelled": "Stopped by you",
			"terminal.exit": "exit",
			"notes.new": "New",
			"notes.delete": "Delete",
		...
		/**
		 * Namespace-bound translator, bound in apply(). Resolving the language at
		 * CALL time is what makes it correct: a one-shot probe taken during
		 * apply() reads the locale service's default, because the user's
		 * preference has not been applied yet — so a deployment preferring zh was
		 * pinned to English for the life of the process, while the panels that
		 * call `ctx.locale.bind()` during render (artifacts, memory) came out
		 * right. `activeLocale` stays as the fallback for a context with no
		 * locale service at all.
		 */
		let boundT = null;
		...
		function TerminalPanel() {
			...
			// The id of the command in flight. A request/response pair cannot
			// cancel itself — the handle to kill only exists inside the host's
			// handler for that one request — so the run is named here and a second
			// request addresses it by name.
			...
			const run = async () => {
				...
				try {
					const res = await fetch("/api/terminal-notes/exec", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ command: line, runId }),
					});
					const data = await res.json();
					if (!data.ok) {
						// A stopped or timed-out command may still have printed
						...
						if (data.error === "timeout") {
							setError(t("terminal.timeout") + ` (${Math.round((data.timeoutMs ?? 0) / 1000)}s)`);
						} else if (data.error === "cancelled") {
							setError(t("terminal.cancelled"));
						} else {
			...
			/**
			 * Stop the command in flight. The exec request stays open and answers
			 * with the `cancelled` result, so `run()` still owns the state
			 * updates — this only asks the host to terminate the child.
			 */
			const cancel = async () => {
				const runId = runIdRef.current;
				if (runId === null || !running) return;
				try {
					await fetch("/api/terminal-notes/exec-cancel", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({ runId }),
					});
				} catch { /* the run will report its own outcome */ }
			};
			const termWrap = {
				display: "flex", flexDirection: "column", gap: 8, height: "100%",
			...
			return react.createElement("div", { style: termWrap },
				react.createElement("div", { style: { display: "flex", gap: 6 } },
					...
					// Only while something is running: the way out of a long
					// command without waiting for the deadline.
					running && react.createElement("button", {
						style: { ...termInput, cursor: "pointer", flex: "0 0 auto", borderColor: "#7a3b3b", color: "#ffb3b3" },
						onClick: cancel,
					}, t("terminal.cancel")),
				),
		...
		function apply(ctx) {
			...
			// The panels take no props, so they share this module-scoped t. Binding
			// it is what makes the language correct; the probe below is kept only
			// for a context with no locale service, and it runs too early to be
			// trusted on its own (see the note on `boundT`).
			try {
