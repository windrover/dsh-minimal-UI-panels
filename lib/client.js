...
//#region lib/types/client/locales.js
		...
		function ArtifactsPanel({ sessionId, useSessions, useWorkspaces, closeDetails, t, embedded }) {
			...
			// Default view = the per-session artifact folders (<workspace>/sessions),
			// which lists every session's own folder; the user can still navigate
			// anywhere (setDir) and drill into a specific session's folder.
			const effectiveDir = dir ?? (baseDir === null ? null : baseDir.replace(/\/+$/, "") + "/sessions");
			react.useEffect(() => {
