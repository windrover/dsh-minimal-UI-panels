...
//#region lib/types/client/locales.js
		...
		function TabsContainer(props) {
			const { t, renderSlot, tabEntries, closeDetails, openDetails, SessionProvider } = props;
			...
			const renderLeaf = (key) => {
				...
				return react.createElement("div", {
					...
					react.createElement("div", {
						...
						style: { display: "flex", alignItems: "center", gap: 4, padding: "3px 6px", fontSize: 12, fontWeight: 600, cursor: "grab", userSelect: "none", background: "rgba(128,128,128,.12)", borderBottom: "1px solid rgba(128,128,128,.2)", color: "var(--dsw-alias-label-primary, inherit)" },
					},
			...
			return react.createElement("div", { style: { display: "flex", flexDirection: "column", height: "100%", width: "100%", minWidth: 0, minHeight: 0 } },
				!open && typeof openDetails === "function" && react.createElement("button", {
					onClick: openDetails,
					title: t("panel.open"),
					"aria-label": t("panel.open"),
					style: {
						position: "fixed", right: 2, top: "50%", transform: "translateY(-50%)",
						zIndex: 2147483000, width: 26, height: 44,
		...
		function DockRail({ t, tabEntries, openDetails }) {
			...
			return react.createElement("div", {
				style: {
					position: "fixed", right: 2, top: 0, bottom: 0, zIndex: 2147483000,
...
//#region lib/types/client/locales.js
		...
		const css = [
			...
			".dap-title{display:flex;align-items:center;justify-content:space-between;font-size:13px;font-weight:700;line-height:18px;color:var(--dsw-alias-label-primary,#1a1a1a)}",
		...
		function ArtifactsPanel({ sessionId, useSessions, useWorkspaces, closeDetails, t, embedded }) {
			...
			const baseDir = cwd ?? (workspaces.length > 0 ? workspaces[0].path : null);
			const sessionsRoot = baseDir === null ? null : baseDir.replace(/\/+$/, "") + "/sessions";
			const parentOfBase = baseDir === null ? null : parentDir(baseDir);
			// A stale prefs.dir (from before this feature shipped, or typed
			// manually) used to override the session-aware default and leak the
			// whole workspace tree into the panel. Only honour a manual dir
			// when it points inside the sessions tree, the workspace root, or
			// the workspace's real parent — otherwise the default per-session
			// list takes over and the user can still drill down into any
			// session folder.
			const safeDir = (dir !== null && dir !== "" && baseDir !== null
				&& (dir === sessionsRoot
					|| (sessionsRoot !== null && dir.startsWith(sessionsRoot + "/"))
					|| dir === baseDir
					|| (parentOfBase !== null && (dir === parentOfBase || dir.startsWith(parentOfBase + "/")))))
				? dir : null;
			const effectiveDir = safeDir ?? sessionsRoot;
			react.useEffect(() => {
								...
								children: [
									(0, react_jsx_runtime.jsx)("select", {
										...
										children: [
											...
											parentOfBase !== null && (0, react_jsx_runtime.jsx)("option", {
												value: parentOfBase,
												children: t("panel.up") + ": " + parentOfBase
											}),
									...
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: "dap-button",
										disabled: parentOfBase === null,
										title: t("panel.up"),
										"aria-label": t("panel.up"),
										onClick: () => {
											if (parentOfBase !== null) setDir(parentOfBase);
										},
										children: "⬆"
									}),
									(0, react_jsx_runtime.jsx)("button", {
		...
		function MemoryPanel(props) {
			...
			const sections = visibleScopes.map((sc) => {
				...
				return react.createElement("div", { key: sc },
					react.createElement("div", { style: { display: "flex", alignItems: "baseline", gap: 6, margin: "8px 0 2px", borderBottom: "1px solid rgba(128,128,128,.25)" } },
						react.createElement("span", { style: { fontWeight: 700, fontSize: 13, color: "var(--dsw-alias-label-secondary, #8a8a8a)" } }, t(`scope.${sc}`)),
			...
			const toolbar = react.createElement("div", { style: { display: "flex", gap: 5, marginBottom: 8, alignItems: "center", flexWrap: "nowrap", minWidth: 0, overflowX: "auto", flex: "0 0 auto" } },
				...
				react.createElement("button", {
					...
					style: { ...btnStyle, background: showSettings ? "rgba(60,200,120,.2)" : undefined, flex: "0 0 auto", padding: "4px 8px", fontSize: 12, lineHeight: 1.4 },
			...
			return react.createElement("div", { style: { padding: 8, fontSize: 12, overflow: "auto", height: "100%" } },
				!embedded && react.createElement("div", { style: { display: "flex", alignItems: "center", fontWeight: 700, marginBottom: 8, paddingBottom: 4, borderBottom: "1px solid rgba(128,128,128,.2)", color: "var(--dsw-alias-label-primary, inherit)" } },
...
const NS = "terminal-notes";
		...
		function TerminalPanel() {
			...
			return react.createElement("div", { style: termWrap },
				...
				!error && react.createElement("div", { style: { ...hintStyle, flex: hintsOpen ? "1 1 0%" : "0 0 auto" } },
					react.createElement("div", {
						style: { fontWeight: 600, marginBottom: hintsOpen ? 4 : 0, color: "#aab4c2", cursor: "pointer", userSelect: "none", display: "flex", alignItems: "center", gap: 6 },
						onClick: () => setHintsOpen(!hintsOpen),
						title: hintsOpen ? "折叠" : "展开",
					},
						react.createElement("span", { style: { fontSize: 10, opacity: .7 } }, hintsOpen ? "▾" : "▸"),
						t("terminal.hintsTitle")),
					hintsOpen && hints.map((h, i) => react.createElement("div", { key: i, style: { display: "grid", gridTemplateColumns: "auto 1fr", gap: "0 10px", alignItems: "baseline" } },
		...
		function NotesPanel() {
			...
			return react.createElement("div", { style: panelStyle },
				react.createElement("div", { style: toolbarStyle },
					react.createElement("button", { style: { ...buttonStyle, alignSelf: "auto", padding: "2px 7px", background: "rgba(60,140,255,.2)", borderColor: "rgba(60,140,255,.4)" }, onClick: createNote }, t("notes.new")),
					react.createElement("button", { style: { ...buttonStyle, alignSelf: "auto", padding: "2px 7px", color: "#e5484d" }, onClick: deleteNote, disabled: !activeId }, t("notes.delete")),
					// 2.3: manual list toggle.
					react.createElement("button", {
						style: { ...buttonStyle, alignSelf: "auto", padding: "2px 7px" },
