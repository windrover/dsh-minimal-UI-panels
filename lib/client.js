...
const NS = "terminal-notes";
		...
		function NotesPanel() {
			...
			const splitterStyle = {
				...
				background: "rgba(128,128,128,.08)", borderRadius: 1,
				alignSelf: "stretch",
			};
		...
		function apply(ctx) {
			detailsTabs(react).apply(ctx);
			artifacts(react).apply(ctx);
			ltm(react).apply(ctx);
			terminalNotes(react).apply(ctx);
		}
