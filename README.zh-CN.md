...
右侧栏里开着**「产物 + 终端」**这一对：上面是产物面板，下面是终端，中间是可拖动的分隔条。标签条上的胶囊名就是这一对的类型名。

<img src="docs/screenshot.png" alt="右侧栏中的「产物 + 终端」组合标签" width="400">

要把四个面板同时放到屏幕上，把这个标签**拖到窗格的左/右边缘**分出第二个窗格，另一格开「长期记忆 + 记事本」即可——两个窗格并排，四个面板各占一格的一半高度。
